...
# CONFIG: Chlorophyll proxy thresholds on the 640-700 nm absorbance band.
# Calibrated against the LOPEX93 dataset (Hosgood et al., 1993) using the
# Beer-Lambert relationship A = -log10(R). Replace these values with the
# slope/intercept and recommended_thresholds emitted by
# calibrate_chlorophyll.py once you've run it on LOPEX93.
CHLOROPHYLL_BAND_NM = (640.0, 700.0)

CHLOROPHYLL_THRESHOLDS = {
    "low_max": 0.50,       # absorbance <= 0.50         -> low
    "moderate_max": 0.80,  # 0.50 < absorbance <= 0.80  -> moderate
                           # absorbance > 0.80           -> high
}

CHLOROPHYLL_NOTE = (
    "Chlorophyll proxy is the mean Beer-Lambert absorbance "
    "(A = -log10(R)) over the 640-700 nm chlorophyll absorption band. "
    "Thresholds calibrated against LOPEX93 (n=330 leaves) at the 33rd/67th "
    "percentiles of measured chlorophyll a+b. Replace via calibrate_chlorophyll.py."
)
...
def chlorophyll_absorbance(wavelengths: np.ndarray, R: np.ndarray) -> float:
    """Mean Beer-Lambert absorbance A = -log10(R) over 640-700 nm.

    Reflectance is clipped to [1e-4, 0.9999] to avoid log(0) and log of >1.
    """
    wavelengths = np.asarray(wavelengths)
    R = np.asarray(R, dtype=float)
    lo, hi = CHLOROPHYLL_BAND_NM
    mask = (wavelengths >= lo) & (wavelengths <= hi)
    if mask.sum() < 1:
        return 0.0
    band_R = np.clip(R[mask], 1e-4, 0.9999)
    return float(-np.log10(np.mean(band_R)))


def chlorophyll_class(absorbance: float) -> str:
    if absorbance <= CHLOROPHYLL_THRESHOLDS["low_max"]:
        return "low"
    if absorbance <= CHLOROPHYLL_THRESHOLDS["moderate_max"]:
        return "moderate"
    return "high"
...
def analyse_spectrum(preset: str,
    ...
    if preset == "general":
        ...
        absorbance = chlorophyll_absorbance(wavelengths, R)
        out["chlorophyll_proxy"] = {
            "absorbance_640_700_nm": absorbance,
            "level": chlorophyll_class(absorbance),
            "thresholds": CHLOROPHYLL_THRESHOLDS,
            "band_nm": list(CHLOROPHYLL_BAND_NM),
